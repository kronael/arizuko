...
| `<inherited from=… through=…>`            | `gateway/gateway.go`           | parent topic's trailing history on a forked child's early turns; capped by `INHERIT_WINDOW_MESSAGES`/`INHERIT_WINDOW_CHARS` (spec 6/F)                                                                                       |
