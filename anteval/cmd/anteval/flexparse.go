...
// Vendored from cmd/arizuko/flags.go — anteval keeps zero arizuko imports
// (spec 11/A), so the helper is copied, not imported. Keep edits in sync.
...
// positionals — unlike std flag, which stops at the first non-flag arg. The
// documented `anteval run <target> --flags…` form put every flag AFTER the
// positional, so std fs.Parse silently dropped them all (no token, no chat,
// --smoke ignored → 19 inject-404s on the first live run, 2026-07-11). It
// reorders args so all flags come first, then delegates the actual parsing
// to fs.Parse. After it returns, positionals are available via fs.Args().
//
// Detection rules:
