...
- **Secrets** — AES-256-GCM encrypted at rest. Operator anchors
  (`ANTHROPIC_API_KEY`/`CLAUDE_CODE_OAUTH_TOKEN`) from host env plus
  ...
  for that spawn. Tool-call-time broker still planned (spec 7/Y).
...
- The operator anchors (`ANTHROPIC_API_KEY`/`CLAUDE_CODE_OAUTH_TOKEN`)
  plus routd-resolved folder secrets are injected into the container at
  spawn; a web-chat trigger's user-scoped (BYOA) keys override the
  folder keys for that spawn (`routd/dispatch.go` `FolderSecretsForUser`
  → `container/runner.go` `mergeSecrets`). The egress allowlist restricts
