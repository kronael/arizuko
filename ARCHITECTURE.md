...
| `onboarding`         | jid (PK), status (`awaiting_message` → `queued`/`approved`/`refused`), prompted_at, user_sub, gate, queued_at, admitted_at (onbod.db). `token_ref`/`token_expires` are inert since the pairing fold — `route_tokens` carries the link (spec 5/31)                                                                                                                    |
