...
| `installed_packages` | name (PK), source, revision, manifest (JSON: asset-kind → owned identities), asset_hashes (JSON: asset-id → content hash), installed_at — one row per installed package; the record `upgrade`/`remove`/dirty-detection read (routd.db, migration 0020, spec 5/28)                          |
