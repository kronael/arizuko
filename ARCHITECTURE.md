...
| `acl_membership`     | child + parent (PK), added_by, added_at — role memberships + JID→sub claims. Two claim writers: a consented pairing redemption stamps `added_by='pairing'` (the only rows `unpair` can delete, spec 5/31); onbod's onboarding claim stamps `added_by='linkJID'` (routd.db, migration 0007)                                                                           |
