...
time; recorded at the cold-tier boundary per `8/5`'s Markdown-hashing
...
exits, redeploys, and worktree forks ([`9/3`](3-git-as-truth.md)
fork lifecycle).
...
Per [`9/3`](3-git-as-truth.md), function manifest rows are cold-tier
...
[`8/F`](../8/F-audit-stream.md). New `tool` values in `audit_log`
...
- `arizuko apply` with a `functions:` block creates table rows + emits
  audit rows; second apply is a no-op (idempotent per `8/5`).
