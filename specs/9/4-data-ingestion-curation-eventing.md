---
status: draft
depends: specs/9/2-data-model.md, specs/9/3-git-as-truth.md
---

# specs/9/4 — data ingestion, curation, eventing in the agent-is-data world
...
mounts WebDAV for human drops; `filewd` (`specs/7/3`) proposes
...
Already flagged in `9/3 ## Hot path`. Restated because every other
...
`PRODUCT.md` today (`specs/17/P-product-templates.md`) declares
...
Q5 ↔ Q8). Resolutions land as separate specs under `specs/15/` or
...
- Does NOT extend `specs/17/3-file-event-stream.md`. That spec is
...
- Does NOT touch the messages-table schema. Hot tier shape is
  `specs/9/2-data-model.md`.
- Does NOT replace `specs/17/8-company-brain.md`. That spec frames
...
- Does NOT presume `agents.toml` exists yet. The composer is drafted
  separately (`specs/9/index.md ## What this phase is NOT`); this
