...
(filed as `specs/11/16-whapd-auth-rotate.md`); this spec is only the
re-pair flow.
...
- NOT a way to bypass WhatsApp's per-account link-rate limit. The
  ...
  enforce its own.
- NOT the auth-dir auto-rotate. See `specs/11/16-whapd-auth-rotate.md`.
...
3. ~~Auto-rotate spec stub.~~ Done — `specs/11/16-whapd-auth-rotate.md`
