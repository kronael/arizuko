...
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| `skills/`                    | union by skill name; collision: LAST product in the mix wins, wholesale (a later mixin overrides an earlier's same-named skill). No cross-product merge — two providers share no merge base, so a git-style merge is undefined here; override is the safe blend. | **managed**: clean replace at new rev (immutable installs, no merge) |
| `PERSONA.md`                 | FIRST product in the mix that provides one wins; later providers ignored (warned)                                                                                                                                                                                | seed-once: group state after create, never touched                   |
