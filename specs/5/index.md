...
| [31-identity-pairing.md](31-identity-pairing.md) | partial | **Channel identity → verified account.** A channel user is anonymous and holds no grants; the `acl_membership` edge is the only bridge, and only the account owner's consent at `/pair/<token>` writes one. A pairing is a `kind='pair'` route token. Redemption now continues to `/onboard`, which routes the JID. The onboarding fold (`P1b` blockers 1–3) is designed and unbuilt. |
