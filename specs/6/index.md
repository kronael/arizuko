...
| [H-oauth-services.md](H-oauth-services.md)             | brainstorm | Third-party OAuth services (Gmail/GCal/Drive/Notion/Linear/GitHub/MS 365/Asana/Jira) as agent capabilities. One generic shape: per-user OAuth → token wallet → off-the-shelf MCP server OR 6/E bridge catalog → `auth.Authorize` gate. Bigger framing: arizuko as agent-GSuite — tenants, identity, capabilities, governance, billing, marketplace as the five primitives. |
| [F-topic-lineage.md](F-topic-lineage.md)               | spec       | Topics carry `parent_topic`, `forked_at`, `observed_cursor`. One primitive unlocks: `fork_topic` MCP, platform threads fork from parent, new topics default-fork from main, per-topic observed cursor (fixes double-feed / never-see). Three columns on `sessions`, ~420 LOC total.                                                                                        |
| [G-engagement.md](G-engagement.md)                     | spec       | Mention starts an engagement on `(jid, topic)`; subsequent inbounds trigger without re-mention until TTL or explicit `disengage()`. Slack channel replies auto-thread by default; corrective exchanges spawn a side thread to keep main flow clean. `<surface>` hint lets agent self-cap per platform. ~225 LOC.                                                           |
| [D-slack-agent-pane.md](D-slack-agent-pane.md)         | spec       | Full Slack AI sidebar support: pane_sessions table; assistant_thread_started/\_context_changed event handlers; setTitle on open; setSuggestedPrompts after every reply; pane_context surfaced to agent prompt; PERSONA.md `pane_title`/`pane_prompts` frontmatter. ~470 LOC, 8 phases.                                                                                     |
