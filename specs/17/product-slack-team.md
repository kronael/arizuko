...
| `SECRETS.toml`                                 | **Vapour today.** The only repo refs are the product HTML page and `specs/11/14-plugins.md`. The actual at-rest store is the `secrets` SQLite table. Treat `SECRETS.toml` as the eventual operator-edit format; ship the product against the table.                 |

## File set to ship

Drop these into `ant/examples/slack-team/`. Mirrors the existing
`support` template structure (`specs/17/P-product-templates.md`).
...
#    channel route — see specs/5/32-acl-unified.md.
...
#    the channel route as the auth fence — see specs/5/32-acl-unified.md.
```
...
1. **Per-user secrets in a Slack channel.**
   ...
   Spec covering this: **`specs/5/13-ext-mcp.md`** (tool-level
...
2. **No CLI / dash UI to set a folder secret.**
   ...
   - Cleanest no-code path: defer secret-setting to the
     forthcoming `specs/5/13-ext-mcp.md` dashboard UX. For now, document the
   ...
   **Recommended template behaviour**: `PRODUCT.md` lists
   ...
   (a) `specs/11/14-plugins.md` (operator UX via `arizuko plugin`),
   or (b) `specs/5/13-ext-mcp.md` (dashboard
...
- `SECRETS.toml` as the canonical operator-edit format vs the
  ...
  reconciler. See `specs/11/14-plugins.md`. Out of scope here.
