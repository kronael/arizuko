---
status: draft
supersedes: specs/13/8-self-eval-skill.md (judge-model choice + data shape)
...
This supersedes the `specs/13/8-self-eval-skill.md` design where the
...
| `flags`     | string[]       | Free-form tags. Stable vocab grows from `7/3` playbooks. |
| `reasoning` | string (≤500c) | Operator-readable; cited by `7/2` when firing triggers.  |

Stable flag vocab to seed (each maps to a repair playbook in `7/3`):
...
`7/2`'s state-evaluator read from this directly — no separate
...
`7/4`-style longitudinal analysis benefits from the long tail. Add a
cleanup job in `timed` only when disk pressure shows.

## Trigger

Sub-`query()` at container exit, NOT a separate cron. (Same trigger
choice as `specs/13/8-self-eval-skill.md`; that part holds.) Fires on
...
- **Routing on score.** The state-evaluator (`7/2`) reads
  `skill_health` and decides what to fire. This spec only writes.
- **Operator UI.** A dashd `/health` page comes in `10/18-daemon-dashboards`
  scope; not blocked by this spec.
- **Auto-revert on low score.** That's `7/3` repair-playbook
  territory.
...
- Storage is one `skill_health` row per turn (or per-skill where
  attributable). No aggregation table — `7/2` aggregates on read.
