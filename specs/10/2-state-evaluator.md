---
status: draft
depends-on: specs/10/1-self-eval-haiku.md
...
dispatches to a skill (typically a repair playbook from `7/3`), and
the loop closes.
...
`/repair-<condition>` skill (see `7/3`). If no playbook matches, the
