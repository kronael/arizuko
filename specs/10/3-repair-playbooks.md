---
status: draft
depends-on: specs/10/2-state-evaluator.md
...
`10/7-self-learning` spec covers the proposal-and-operator-approval
...
| -------------------- | ------ | ------------------------- |
| Immediate response   | `7/3`  | Agent (skill dispatch)    |
| Persistent learnings | `11/7` | Operator (proposal queue) |
...
- **Auto-rollback on bad deploys** — needs `13/d-agent-code-modification`
  to land first.
...
- Playbooks observe and suggest; they don't auto-edit persona/skills/
  permissions. Durable changes flow through `11/7-self-learning`.
