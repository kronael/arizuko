---
status: draft
references: specs/17/9-positioning.md
...
This goes hand-in-hand with `13/f-replaceability-research.md` (which
...
## Updates to `specs/17/9-positioning.md`
...
3. The replaceability story (specs/14/f) and the
   agent-managing-agents story (specs/17/9) compose.
```
...
## Drop `specs/13/6-workflows.md` (workflowd)
...
1. Mark `specs/13/6-workflows.md` as superseded.
...
Three messaging anchors `specs/17/9` should add:
...
- `specs/17/9-positioning.md` gets a "Why componentized matters"
  section + an updated "What arizuko Is" line. This spec is the
  source.
- `specs/13/6-workflows.md` (workflowd) is recommended for supersede.
  The use cases collapse into "make a folder".
- Daemons are evaluated against two questions: (1) could we drop
  this for an off-the-shelf alternative? (`13/f`), and (2) could
  we replace this with an agent doing the same job? (this spec).

## References

- `specs/17/9-positioning.md` — the doc this updates.
- `specs/15/f-replaceability-research.md` — sibling axis.
- `template/web/pub/concepts/web-native-agents.html` — the prior
  framing of "automations are folders".
- `specs/13/6-workflows.md` — the spec this recommends superseding.
