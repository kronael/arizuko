...
**Decision**: import as `/watch-price` with cooldown via `7/2`'s
table.

### `/github-pr-summary` — IMPORT (HIGH PRIORITY)

**Aeon**: reads a PR diff + comments, writes a summary.
**Fit**: synergy with `specs/17/1-git-channel.md` (git-as-channel).
The PR thread is a folder; the summary skill is a folder operation.
**Effort**: small once `8/1` lands; medium otherwise.
...
heartbeat.
**Fit**: arizuko has `skill_health` via `7/1`. The Aeon pattern is a
weaker version of what `7/1` ships.
**Decision**: skip; `7/1` is the better answer.
...
`8/product-personal-assistant.md`) wants this.
...
**Fit**: arizuko has `specs/5/13-ext-mcp.md`. Aeon's pattern is
