...
- Stored encrypted at rest using the same AES-GCM-from-AUTH_SECRET
  scheme proposed for `creds_json` in `12/6-dynamic-channels.md`.
...
The `channels` table from `12/6-dynamic-channels.md` (if shipped)
...
- **Depends on** `12/6-dynamic-channels.md` for the channel-row +
