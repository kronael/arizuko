...
[`7/1`](1-cockpit-index.md). This spec adds only routd's page list,
...
equivalent of the `[[proxyd_route]]` TOML, per `7/1` Routing).
...
`7/2`'s migration rule the migrating PR deletes the dashd renderer and
repoints the link.
...
**queue** — live process state only routd can serve (the `7/1`
read-path argument):
...
- breaker pane: chats at the threshold, with the trip evidence — the
  ...
  render on `7/4`; here we show the routd-side consequence.
...
- the `auth/dashauth.go` CSRF check (`7/1`).
...
dashboard (`7/1`: no dashboard-only API).

## 6. Auth

Per `7/1` exactly: proxyd transit gate (`auth:"user"`) → daemon-side
...
name (`7/1` Theme), nothing else.

## 8. Non-goals

`7/1` non-goals apply. routd-specific:
...
- **No message browsing.** dashd's activity page owns that (`7/2`).
- **No cost time-series.** Today-vs-cap and recent rows only (`7/1`:
  not a metrics system).

## 9. Acceptance

- `GET /dash/routd/` behind proxyd renders the overview for an
  operator; a non-operator gets the themed 403 banner (`7/1` Auth).
...
- All page data flows through `/v1` handlers/helpers — no SQL in any
  `/dash/` handler (code-review check, `7/1` read-path).
