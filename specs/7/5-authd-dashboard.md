...
Per `7/1`: proxyd `auth:"user"` transit on `/dash/authd/` +
...
faces above exist for dashd's cross-cutting pages (`7/2` ACL/profile
...
Per `7/1`, plus: no JWT decoder/inspector for arbitrary pasted tokens;
...
editing (grants belong to routd's ACL, surfaced in `7/2`); no
