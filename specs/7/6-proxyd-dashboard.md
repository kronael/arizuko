...
Architecture, routing, auth, theme per [`7/1`](1-cockpit-index.md).
...
- **Denial ring**: a fixed-size in-memory ring buffer (e.g. last 200)
  - per-class counters, appended at the four emit sites above. Exposed
    ...
    (reset). Live process state — exactly the `7/1` rationale for
...
- **Upstream probe**: `GET /v1/upstreams` — for each **distinct**
  ...
  `7/1` hub probing). Response: backend, ok/err, latency, the routes
...
Per `7/1`: proxyd carries its own `/dash/proxyd/` route (`auth:
...
two faces (`7/1` read-path), not a parallel non-`/v1` path — leaving
...
Per `7/1`, plus: no access-log browser (journald/audit owns request
