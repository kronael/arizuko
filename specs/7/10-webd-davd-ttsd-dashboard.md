...
Architecture, routing, auth, theme: [`7/1`](1-cockpit-index.md). Three
...
and they render on routd's routes page ([`7/3`](3-routd-dashboard.md)).
...
- **Overview** — channel registration health (the `/health` payload,
  ...
  exactly the live-state argument of `7/1`.

### Control

**None.** Token revocation and web-route edits live on routd's routes
page (`7/3`); webd holds no mutable runtime state of its own. No token
...
- **Show** — tile health only. dufs has no `/health`; the probe is
  ...
  needs a per-tile probe-path override in the hub — flagged to `7/2`
...
Per `7/1` for webd and ttsd: proxyd `auth: "user"` transit +
...
Per `7/1`. Additionally: no chat-message browsing in webd's dash (the
...
- No `/dash/webd/tokens` or `/dash/webd/routes` page exists; route
  tokens and web routes render on routd's routes page (`7/3`).
