...
([`7/11`](11-adapter-contract.md)) for the three mixed gateway
...
- Guild/channel cache **counts** from discordgo `session.State` —
  ...
  refresh API in use, so no refresh-cache control (absent, per `7/11`).
...
- Thread store: row counts of `email_threads` / `email_msg_ids`,
  ...
  (`7/1` read-path: `/v1`-only, no exceptions).
...
- Cookie session state: cookies present + mtime, login validity (last
  ...
  `cookies.json` / `cursors.json` directly (`7/1` read-path).
...
- `twitd/src/server.ts` verify only routd service tokens. Re-implement
  ...
  `theme/` at image build, per `7/11` "TS adapters".

## Required `/v1` work

Beyond the shared chanlib work in `7/11`:
...
Per [`7/11`](11-adapter-contract.md) and [`7/1`](1-cockpit-index.md).
...
`7/13`); no Twitter posting UI.

## Acceptance

- Contract acceptance (`7/11`) passes for all three.
