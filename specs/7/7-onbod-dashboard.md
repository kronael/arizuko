...
Per `7/1`: proxyd `auth:"user"` transit on `/dash/onbod/` +
...
Per `7/1`, plus: no cross-table user browsing (auth_users/acl/groups
are routd/authd-owned cross-reads — the user-profile view is dashd's
cross-cutting page, `7/2`); no editing of `ONBOARDING_GATES` env
