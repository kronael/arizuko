...
[`7/1`](1-cockpit-index.md). This spec adds only runed's page list,
...
goes in the static core list in `compose/compose.go`, per `7/1`
...
**overview** — live `Manager` state only the daemon can serve (`7/1`
...
authd concern (`7/5`); duplicating it here would be a second renderer
...
`?state=` filter through. Theme tables + detail panes by name (`7/1`).

## 8. Non-goals

`7/1` non-goals apply. runed-specific:

- **No token revocation verb** — kill-run is the revoke (above); jti
  revocation belongs to authd (`7/5`).
...
- `GET /dash/runed/` behind proxyd renders capacity for an operator;
  non-operator gets the themed 403 banner (`7/1` Auth).
