...
([`7/11`](11-adapter-contract.md)) for the four stream/poll social
...
- Cursor table (extra section): per-source cursor state surfaced via
  ...
  `cursors.json` read from a dash handler, `7/1` read-path) — `inbox`
...
- Contract acceptance (`7/11`) passes for all four.
