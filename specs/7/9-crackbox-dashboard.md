...
Architecture, routing, auth, theme: [`7/1`](1-cockpit-index.md). This
...
1. **Theme**: crackbox imports `github.com/kronael/arizuko/theme` —
   ...
   (violates `7/1` per-daemon ownership).
...
- **Normal lifecycle needs neither**: runed registers per spawn and
  ...
  container is a `7/4` concern — here, operator judgment) first.
- **Ephemerality, stated in the UI**: a live-allowlist edit lasts until
  ...
  routd's dashboard (`7/3`), **not here**. Crackbox's dash shows live
...
Gate 1 per `7/1` but `auth: "operator"` at proxyd (crackbox cannot run
...
Per `7/1`. Additionally: no persistent egress-policy editing (routd's,
`7/3`); no KVM/host-half views (nothing shipped); no per-request
