...
- Stored encrypted at rest using the same AES-GCM-from-AUTH_SECRET
  scheme proposed for `creds_json` in `13/6-dynamic-channels.md`.
  This spec does not redefine the scheme; if `8/32` ships first,
  reuse; if this ships first, define the helper here and `8/32`
  reuses it.
...
The `channels` table from `13/6-dynamic-channels.md` (if shipped)
is where status flips from `pairing` to `active`. Without `8/32`,
...
If `8/32` ships first, this is one new button on the existing card.
If this ships first, dashd grows a minimal channel list scoped to
the daemons that opt in.

## Cross-references

- **Depends on** `13/6-dynamic-channels.md` for the channel-row +
  encrypted-creds infrastructure. If `8/32` is not yet shipped,
  this spec ships a minimal subset (in-memory channel registry,
  per-daemon creds dir) and migrates to `channels` rows later.
- **Complements** `5/5-tenant-self-service.md`: tenant-controlled
...
- **Reuses** `auth/hmac.go` for token signing, `auth/oauth.go`
  ...
  routing layer, `dashd` channels page (per `8/32`).
...
- Multi-account-per-channel (two X accounts on one instance) —
  separate concern; needs `8/32`'s channel-rows model.
...
- Encrypted-at-rest storage — covered by `8/32`. If this ships
  first, define a minimal helper and let `8/32` adopt.
...
- How does an operator revoke a tunneled credential? Delete the
  channel row (per `8/32`) is the obvious answer; document.
