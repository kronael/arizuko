...
The CLI command and the dashd web UI (per `8/37-auth-tunneling`) are
...
3. **`oauth_redirect`** — daemon participates in a one-shot OAuth
   flow via `auth-tunnel` (per `8/37`). CLI calls dashd's
...
1. POSTs to dashd's `/auth-tunnel/<channel>/begin` (per `8/37`)
...
`oauth_redirect` once `8/37` lands.

## Open questions

1. Should `--revoke` also unregister the channel row (per `8/32`),
