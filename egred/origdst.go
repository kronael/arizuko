	...
	err = rc.Control(func(fd uintptr) {
		if ipv6 {
			...
			_, _, errno := syscall.Syscall6(
				...
				uintptr(unsafePtrOf(&sa6)), uintptr(unsafePtrOf(&sz)), 0)
			if errno != 0 {
			...
			_, _, errno := syscall.Syscall6(
				...
				uintptr(unsafePtrOf(&sa4)), uintptr(unsafePtrOf(&sz)), 0)
			if errno != 0 {
