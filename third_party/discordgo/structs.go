...
type Session struct {
	...
	// RawIdentifyData, when non-empty, replaces the default Identify
	// payload sent on the gateway. Used to send user-account shaped
	// IDENTIFY (capabilities, browser properties, client_state) which
	// the Identify struct cannot represent. The bytes are the full
	// `d` object of OP 2.
	RawIdentifyData json.RawMessage
