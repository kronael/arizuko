...
func (s *Session) identify() error {
	...
	// User-mode override: if RawIdentifyData is set, send it verbatim
	// as OP 2 instead of the default bot-shaped Identify struct. The
	// default Identify carries Intents=0 with no omitempty, which the
	// user gateway interprets as "opt into intent filtering with no
	// intents granted" — collapses MESSAGE_CREATE to mentions only.
	if len(s.RawIdentifyData) > 0 {
		raw := struct {
			Op   int             `json:"op"`
			Data json.RawMessage `json:"d"`
		}{2, s.RawIdentifyData}
		s.log(LogDebug, "Raw Identify Packet (user mode)")
		s.wsMutex.Lock()
		err := s.wsConn.WriteJSON(raw)
		s.wsMutex.Unlock()
		return err
	}
