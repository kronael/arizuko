...
- **onbod gates REST face on resreg; phantom path fixed.** `GET/PUT/DELETE /v1/gates` fold onto a shared resreg handler; Endpoints single-sourced at the real `/v1/gates` path, killing the phantom `/v1/onboarding_gates` OpenAPI entry. (`4bd09532`)
- **secrets REST face on resreg — write-only, no audit leak.** `POST/DELETE /v1/secrets` ride a shared handler as a forwarder (`Store: nil`) so resreg's params-summary audit never sees a plaintext value; operator containment (`secrets:write` + own-folder/own-sub) closes a pre-fold bearer-only hole; write-only on `/openapi.json`, no phantom GET. (`0ae1ffa8`)
- **Truthful Endpoints OpenAPI.** `/openapi.json` emits each resource's real mounted `Endpoints` (the same slice `RegisterREST` mounts) instead of a fixed PK-CRUD convention, so handler and doc cannot drift — phantom `PATCH`/`DELETE /v1/routes/{seq}` gone, real `PUT`/`DELETE`-by-id present. (`7c14efd6`)
- **dashd tool browser renders the facade tools.** The cold-tier facade tools' MCP metadata (`MCPDoc`/`MCPArgs`/`MCPNames`) is single-sourced in `resreg/resources`, and `ipc.ListTools` walks the resreg registry with the same grant-visibility filter the agent socket uses — the browser matches the live agent's surface. (`df9ebad3`, `d5023c60`)

### Security

- **No default root; root via operator `/root` elevation.** No group spawns as root by folder shape any more: a NAMED top-level world resolves to tier 1, and root (tier 0, cross-group `*`) is a transient elevation gated on the caller's `**` operator grant (`/root` steer command), flowing an unforgeable `Elevated` flag through dispatch → runed → container. `/migrate` becomes per-group self-migrate — no root fan-out. (`aa4d9d4e`)
- **Cross-tenant `web_routes` delete + task read (fixed).** A NAMED tier-0 tenant could delete — then re-claim — a sibling tenant's web route (the delete scope widened on tier, not on the operator's empty-folder claim), and any `tasks:read` holder could read any task by ID (`GET /v1/tasks/{id}` skipped the containment its DELETE/PATCH siblings run). Both now bind to the caller's folder, with fail-on-broken leak guards; a REST test that codified the old leak was replaced. (`ad44b081`, `48aebca0`)
...
- **Surrogate refresh: bodyless 4xx is transient, not a reconnect.** A transient bodyless 429/4xx no longer nulls the credential and forces a needless reconnect — only a definitive OAuth error body (e.g. `invalid_grant`) signals reconnect; the still-valid refresh token is kept. (`e813efd5`)
