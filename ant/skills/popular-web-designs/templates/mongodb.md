...
> ```
> Use the `Write` tool to create HTML under `~/public_html/<name>/index.html` (served at `/pub/<folder>/<name>/`, no tunnel needed).
> Verify visual accuracy by screenshotting the live `/pub/` URL with the `agent-browser` skill.
