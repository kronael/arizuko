...
description: >
  54 production-quality design systems extracted from real sites (Stripe,
  Linear, Vercel, Notion, Airbnb, …). Load a template for the colors,
  typography, components, spacing, and exact CSS values to generate
  HTML/CSS matching a site's visual identity. USE for "make it look like
  stripe", "design like linear", "vercel style", "build a landing page",
  "dashboard design", "style this like <site>". NOT for arizuko's own docs
  site (visuals are fixed — see template/web/CLAUDE.md) or data charts
  (use tufte).
user-invocable: true
---

<!-- CREDITS: adapted from NousResearch/hermes-agent
     skills/creative/popular-web-designs (MIT; design systems sourced from
     VoltAgent/awesome-design-md). The 54 templates are unchanged; the
     load / serve / verify steps are adapted to the arizuko runtime
     (Read, ~/public_html web slots, agent-browser). -->
...
2. Load it with `Read`: `~/.claude/skills/popular-web-designs/templates/<site>.md`
3. Use the design tokens and component specs when generating HTML
4. **Serve it on your web slot** — write to `~/public_html/<name>/index.html`
   (public, `/pub/<folder>/<name>/`) or `~/private_html/<name>/` (JWT,
   `/priv/<folder>/<name>/`). Verify the URL returns 200/401 before
   announcing (see the publish rules in CLAUDE.md). No tunnel needed.

Each template's notes block carries:
- A CDN font substitute + Google Fonts `<link>` tag (ready to paste)
- CSS font-family stacks for primary and monospace
- Use the `Write` tool for the HTML; verify the rendered look with the
  `agent-browser` skill (headless chromium screenshot of the `/pub/` URL)
...
Write the file with the `Write` tool to `~/public_html/<name>/index.html`,
then confirm it serves: `curl -sI https://$WEB_HOST/pub/$ARIZUKO_GROUP_FOLDER/<name>/ | head -1`
(MUST be 200). For visual accuracy, screenshot the live URL with the
`agent-browser` skill.
