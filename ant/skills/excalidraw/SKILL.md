...
name: excalidraw
description: >
  Author hand-drawn-style diagrams as Excalidraw JSON — architecture
  diagrams, flowcharts, sequence diagrams, concept maps, mind maps. USE
  for "draw a diagram", "flowchart", "architecture diagram", "sketch
  this system", "excalidraw". Outputs a `.excalidraw` file the user opens
  at excalidraw.com. NOT for data charts (use tufte), math animations
  (use manim-video), or text banners (use ascii-art).
user-invocable: true
---

<!-- CREDITS: adapted from NousResearch/hermes-agent
     skills/creative/excalidraw (MIT). Element-format reference and color
     tables unchanged; file I/O + delivery adapted to the arizuko runtime
     (Write tool, send_file, fixed upload-script path). -->
...
2. **Save the file** with the `Write` tool, under `~/tmp/` or `~/workspace/`
3. **Deliver it** with `send_file` (caption = what the diagram shows), or
   optionally upload for a shareable link (below)
...
Save to a `~/`-rooted path, e.g. `~/tmp/my_diagram.excalidraw`, then
`send_file ~/tmp/my_diagram.excalidraw caption="<one-line summary>"`.
The user opens it by dragging onto excalidraw.com.

### Uploading for a Shareable Link (optional)

The bundled script encrypts and uploads to excalidraw.com (no account)
and prints a shareable URL. Requires the `cryptography` package and
`excalidraw.com` on your crackbox egress allowlist:

```bash
uv pip install --system cryptography -q
uv run --python 3.14 ~/.claude/skills/excalidraw/scripts/upload.py ~/tmp/my_diagram.excalidraw
```

If the host is denied (403 on every path) the upload fails — fall back
to `send_file` and tell the user to open the file at excalidraw.com.
