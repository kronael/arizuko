...
name: p5js
description: >
  Interactive + generative visual art with p5.js — generative art,
  creative coding, canvas animations, 3D/WebGL scenes, GLSL shaders,
  particle systems, flow fields, audio-reactive visuals, kinetic
  typography. Outputs a self-contained HTML sketch (CDN p5.js, no build),
  with PNG/GIF/MP4 export. USE for "p5.js sketch", "creative coding",
  "generative art", "canvas animation", "shader effect", "make
  something visual/animated". NOT for data charts (use tufte) or math
  explainers (use manim-video).
user-invocable: true
---

<!-- CREDITS: adapted from NousResearch/hermes-agent skills/creative/p5js
     (MIT). References + scripts + viewer template unchanged; the serve /
     preview / export / delivery steps are adapted to the arizuko runtime
     (chromium headless, ~/public_html web slots, send_file). -->

# p5.js Production Pipeline

## Runtime (arizuko)

- p5.js loads from CDN — needs `cdnjs.cloudflare.com` on your egress
  allowlist (root/world can `network_allow` it). The sketch HTML is
  self-contained otherwise; no build step.
- `chromium`, `node`/`bun`, and `ffmpeg` ship in the image. There is no
  display — preview/export is HEADLESS. To "open in a browser" use
  `chromium --headless` via `scripts/export-frames.js` (Puppeteer drives
  the bundled chromium); never `open`/`xdg-open` (no GUI, no macOS).
- Read reference files with the `Read` tool (e.g.
  `~/.claude/skills/p5js/references/visual-effects.md`), not `skill_view`.
- **Deliver**: a still/GIF/MP4 via `send_file` (caption = what it shows),
  OR publish the interactive sketch to your public web slot —
  `~/public_html/<name>/index.html` → `/pub/<folder>/<name>/`. Verify the
  URL returns 200 before announcing (see the publish rules in CLAUDE.md).
...
- No GUI display in the container — "preview" means a headless screenshot.
  Capture one with the bundled Puppeteer script (it launches the image's
  chromium): `node ~/.claude/skills/p5js/scripts/export-frames.js
  sketch.html --frames 1`, then `Read` the PNG to inspect it.
- For `loadImage()`/`loadFont()` from local files, serve over HTTP first:
  `python3 -m http.server 8080` (or `scripts/serve.sh`) in the sketch dir.
- Test at the target export resolution, not just the window size.
...
When building p5.js sketches (no GUI — everything headless):

1. **Write the HTML file** — single self-contained file, all code inline,
   under `~/tmp/` or `~/workspace/` (use the `Write` tool)
2. **Local assets** (fonts, images) require a server: `python3 -m
   http.server 8080` in the project directory
3. **Headless screenshot / export** — `node
   ~/.claude/skills/p5js/scripts/export-frames.js sketch.html --frames N`
   drives the image's chromium via Puppeteer (sketch must use `noLoop()`
   + `_p5Ready` for >1 frame); `Read` the PNG to inspect quality
4. **MP4 rendering** — `bash ~/.claude/skills/p5js/scripts/render.sh
   sketch.html output.mp4 --duration 30`
5. **Deliver** — still/GIF/MP4 via `send_file caption="…"`, or publish the
   interactive HTML to `~/public_html/<name>/index.html` and send the
   verified `/pub/<folder>/<name>/` URL
6. **Load references on demand** — `Read`
   `~/.claude/skills/p5js/references/<file>.md` as needed during
   implementation
