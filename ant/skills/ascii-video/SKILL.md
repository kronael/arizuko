...
name: ascii-video
description: >
  Render ASCII-art VIDEO (MP4 / GIF / image sequence) from
  video/audio/image/generative input — video-to-ASCII conversion,
  audio-reactive music visualizers, generative ASCII animations,
  text/lyrics overlays, matrix-style effects. USE for "ASCII video", "text
  art video", "terminal-style video", "ASCII audio visualizer", "convert
  this video to ASCII", "matrix effect". NOT for a static ASCII banner or
  image (use ascii-art).
user-invocable: true
---

<!-- CREDITS: adapted from NousResearch/hermes-agent
     skills/creative/ascii-video (MIT). Pipeline + reference docs
     unchanged; deps + delivery noted for the arizuko runtime. -->

# ASCII Video Production Pipeline

## Runtime (arizuko)

- `ffmpeg` ships in the image; NumPy/SciPy/Pillow are pre-installed.
  Add anything missing per-turn with `uv pip install --system <pkg> -q`
  (e.g. `opencv-python-headless`). Run pipeline code with
  `uv run --python 3.14 <script>.py` from `~/tmp/` or `~/workspace/`.
- Read reference files with the `Read` tool
  (`~/.claude/skills/ascii-video/references/<file>.md`).
- Deliver the rendered MP4/GIF with `send_file caption="<what it shows>"`.
  Large renders: write to `~/tmp/` (never `/tmp/` — wiped at turn end).
- The optional ElevenLabs narration path needs an API key + egress; skip
  it unless the operator configured one.
