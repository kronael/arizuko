...
name: manim-video
description: >
  Mathematical + technical animations with Manim Community Edition —
  3Blue1Brown-style explainers, equation derivations, algorithm
  walkthroughs, data stories, architecture build-ups, 3D scenes. Outputs
  an MP4. USE for "animate this concept", "math animation", "explainer
  video", "algorithm visualization", "3Blue1Brown style". NOT for static
  charts (use tufte), browser sketches (use p5js), or diagrams (use
  excalidraw).
user-invocable: true
---

<!-- CREDITS: adapted from NousResearch/hermes-agent
     skills/creative/manim-video (MIT). Manim reference docs unchanged;
     setup, rendering, and delivery adapted to the arizuko runtime
     (uv-install manim, LaTeX fail-soft, send_file). -->

# Manim Video Production Pipeline

## Runtime (arizuko)

- `ffmpeg` ships in the image. **Manim and LaTeX do NOT.** Install Manim
  per-turn: `uv pip install --system manim -q` (or `uvx --from manim
  manim ...`). It pulls a large dependency tree — needs PyPI egress.
- **No system LaTeX.** `Tex`/`MathTex` mobjects (which shell out to
  `pdflatex`) will fail. Use `Text` / `MarkupText` for labels instead, or
  ask root/world to bake `texlive` into the image. Fail soft — do not
  block the whole render on a missing equation; substitute `Text`.
- Render headless to MP4, then deliver with `send_file
  caption="<what it shows>"`. Read reference files with `Read`
  (`~/.claude/skills/manim-video/references/<file>.md`), not `skill_view`.
- `scripts/setup.sh` is a prereq checker — run it first; it reports
...
Run `bash ~/.claude/skills/manim-video/scripts/setup.sh` to verify dependencies. Requires: Python 3.10+, Manim Community Edition v0.20+ (`uv pip install --system manim`), ffmpeg (in the image). LaTeX is OPTIONAL and NOT in the image — only needed for `Tex`/`MathTex`; without it, use `Text`/`MarkupText` (see the runtime note above). Reference docs tested against Manim CE v0.20.1.
