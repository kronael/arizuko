...
// childJID> by copying the parent's prototype/ dir, then registers the group + a
// room-matched route. Honors the parent's max_children gate and rolls the group
// row back if the route insert fails so a route-less orphan is never left behind.
