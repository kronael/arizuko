...
func TestInspectToolsRegistered(t *testing.T) {
	...
	if srv := buildMCPServer(gated, db, "world", true, ""); srv == nil {
		t.Fatal("tier-0 server nil")
	}
	// tier-2 — scoped to own folder
	if srv := buildMCPServer(gated, db, "world/a/b", true, ""); srv == nil {
	...
	if srv := buildMCPServer(gated, dbNoIdent, "world", true, ""); srv == nil {
